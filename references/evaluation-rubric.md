...
| "No sponsorship" clauses | NOT automatically an exclusion | Only matters if the candidate needs sponsorship — check the ledger. If they don't need it, this is actually an advantage; note it in the report |
...
| **Stale posting: > 90 days old, or first-published in a prior calendar year, or undated evergreen-pipeline** | Almost always filled/ghost — a no-send even if the ability match is perfect | Greenhouse rescue: keep if `updated_at` ≤ ~21 days (actively refreshed). Ashby/Lever have no update signal. |
...
1. Extract from the full JD: every **required** concrete item (skills, tools, task types, responsibilities) and every **preferred** item. Concrete = a thing one can have evidence for ("build RAG pipelines", "Python", "design evals", "customer-facing demos").
2. Map each item to the facts ledger:
   - **Hit** — a fact entry is direct evidence the candidate has done this. Cite ID(s).
   - **Partial** — adjacent evidence: the candidate has done a close neighbor of it. Cite ID(s) **and** name the delta (e.g., "Kubernetes missing, but Docker Compose deployment [PROJECT-ID] → partial").
...
3. Anti-inflation rules:
   ...
   - One fact ID can support multiple requirements, but stretchy mappings (e.g. "led a negotiation" → "enterprise sales experience") must be labeled partial, not hit.
   - When honestly uncertain whether partial or missing → choose missing. Strictness bias is the contract.

## Step D — Verdict

- **Tier S (strong)**: every required item is a hit; preferred items mostly ≥ partial. Apply today.
- **Tier A (qualified)**: every required item ≥ partial, zero required items missing. Worth applying; gaps listed.
...
**Both S and A are applyable and both belong in the report's main list — there is NO cap on how many.** Tier is a strength signal for triage, not a filter. Do not create a "passed but not featured" bucket that hides qualifying jobs; if it cleared the bar AND is fresh (see Step A staleness rule), it goes in the list. Only REJECTs and stale postings stay out (they go to `seen.tsv` + an optional sample section). **Order the list freshest-first** (most recent post date at the top — job-hunters think date-first); show the tier per row so the candidate can still weigh strength, but freshness drives the ordering. Never truncate the fresh+qualifying set.

Domain-experience requirements ("3+ years in fintech") stated as *required*: treat as a required item → the candidate will usually be missing it → reject. If merely *preferred*, list as a gap instead.
...
> Date convention (use the **true first-publish date**, don't dress up "recently refreshed" as "just posted"): **Ashby** = `publishedAt`; **Lever** = `createdAt`; **Greenhouse** = `first_published` (note: this field is only returned by the **single-job endpoint** `/boards/{slug}/jobs/{id}`, not the list endpoint `/jobs` — fetch it per-job). Keep Greenhouse's `updated_at` as a separate "last updated" column. First-published >3 months ago and still open = evergreen pipeline, flag it.

## Per-job detail

### 1. <Company> — <Title> (Tier S)
- **Link**: <apply URL>  **Location**: …  **Posted**: …
- **Why the candidate can do this**: 2–3 sentences, citing fact ID(s)
- **Requirement mapping**: `req item → hit (PROJECT-01)` / `req item → partial (PROJECT-15, delta: …)` …
- **Gap / interview prep**: …
- **Recommended resume**: `<YourName>_<family>_2026` + one-line reason

## Today's rejects (optional, 3–5 representative REJECTs with reasons)

## Next steps
Pick jobs → run your resume-tailoring workflow with the posting URL.
```

Chat summary after writing the report: the overview table (or its gist) + where the report file is + the resume-tailoring handoff line. Keep it short.
